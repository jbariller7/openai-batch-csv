...
function ProjectWorkspace({ project, updateProject, isActive }) {
  const { 
    id, name, inputCol, prompt, contextDoc, model, chunkSize, reasoningEffort, mode, 
  ...
  // Calculate correct download endpoint based on the array of batches
...
// Extracted Form Component to keep Workspace clean
function ConfigForm({ file, setFile, isDragging, setIsDragging, update, inputCol, prompt, contextDoc, model, chunkSize, mode, maxRows, concurrency, isCachedHit, estTokens, barWidth, submitBatch, isSubmitting }) {
  return (
    <form onSubmit={submitBatch}>
      ...
      <div className="form-group"><label>Input Column Name</label><input value={inputCol} onChange={(e) => update({inputCol: e.target.value})} /></div>
...
// MAIN APP CONTAINER (Manages Project List)
export default function App() {
  const [projects, setProjects] = useState(() => {
    const saved = localStorage.getItem("batch-csv-projects");
    if (saved) { try { return JSON.parse(saved); } catch (e) {} }
    return [{ id: generateId(), name: "New Project", inputCol: "text", prompt: "Translate the user input into English.", contextDoc: "", model: "gpt-5.4-nano", chunkSize: 500, reasoningEffort: "medium", mode: "batch", concurrency: 4, maxRows: "", batchIds: [], status: "", jobStats: { completed: 0, total: 0 }, analysis: null, lastRunMode: "batch" }];
  });
  ...
  const addProject = () => {
    const p = { id: generateId(), name: "New Project", inputCol: "text", prompt: "Translate...", contextDoc: "", model: "gpt-5.4-nano", chunkSize: 500, reasoningEffort: "medium", mode: "batch", concurrency: 4, maxRows: "", batchIds: [], status: "", jobStats: { completed: 0, total: 0 }, analysis: null, lastRunMode: "batch" };
